...
# Data I/O ---------------------------------------------------------------------
# get adhd dbd data
data_amy  <- readRDS(file.path("data", "preprocessed", "data_amy_shen.rds"))
fconn_amy <- data_amy %>% select(starts_with("amy"))
data_ipl  <- readRDS(file.path("data", "preprocessed", "data_ipl_shen.rds"))
fconn_ipl <- data_ipl %>% select(starts_with("ipl"))

fconn <- cbind(fconn_amy, fconn_ipl)
ext   <- data_amy %>% select(ADD011:ODD006)
data <- cbind(data_amy %>% select(SUBJID:DBD), fconn)

# exclude a subject with missing data
fconn <- fconn[-128, ]
ext   <- ext[-128, ]
data  <- data[-128, ]
...
# Data Partition ---------------------------------------------------------------
set.seed(1111)
tridx <- createDataPartition(data$EXT, p = 0.667, list = FALSE)
data_train <- NULL
...
expectedEV <- 1 / nItems
maxEV <- ceiling(max(EVs) * 100) / 100 + 0.01
...
EVs_screeplot
ggsave(file.path("figures", "dbd_scca_train_screeplot.png"), EVs_screeplot, width = 6, height = 4, dpi = 400)
...
label3 <- sprintf("italic(p[bonf]) == %0.3f", scca_perm_tr_pval_adj[scca_perm_tr_pass[dim.match[3]]])
corrplot <- scca_corr_reorder %>% 
  mutate(ymax = .$rval + .$se) %>%
  mutate(ymin = .$rval - .$se) %>%
  ggplot(aes(neworder, rval)) +
  geom_bar(width = 0.75, stat = "identity", fill = c("gray20", "gray20", "gray20")) +
  ...
  annotate("text", x = 3, y = scca_corr_reorder$rval[3] + 0.02, label = label3, parse = TRUE, size = 4) +
  coord_cartesian(ylim=c(0.2, peak))
corrplot
ggsave(file.path("figures", "dbd_train_corrplot.png"), corrplot, width = 4, height = 3, dpi = 400)
...
a <- bootplot4beh(scca_cand_tr$v[, scca_perm_tr_pass[dim.match[1]]], scca_boot.v[[dim.match[1]]], item_labels)
b <- bootplot4beh(scca_cand_tr$v[, scca_perm_tr_pass[dim.match[2]]], scca_boot.v[[dim.match[2]]], item_labels)
c <- bootplot4beh(scca_cand_tr$v[, scca_perm_tr_pass[dim.match[3]]], scca_boot.v[[dim.match[3]]], item_labels)
#tmp <- ggarrange(a$plot + rremove("x.text"), b$plot+ rremove("x.text"), nrow = 2)
v_plot <- ggarrange(
  a$plot + rremove("x.text"), b$plot+ rremove("x.text"),
  c$plot + rremove("x.text"), nrow = 3) %>%
  ggexport(
    filename = file.path("figures", "dbd_beh_features.pdf"),
    width    = 5,
    height   = 6
  )

ggsave(file.path("figures", "dbd_mode1_behavior.png"), plot = a$plot + rremove("y.text"), height = 2, width = 3)
ggsave(file.path("figures", "dbd_mode2_behavior.png"), plot = b$plot + rremove("y.text"), height = 2, width = 3)
ggsave(file.path("figures", "dbd_mode3_behavior.png"), plot = c$plot + rremove("y.text"), height = 2, width = 3)
...
pdf(file.path("figures", "dbd_network_mode1_features.pdf"), width = 6, height = 6)
sumNet2 %>%
  t() %>% 
  as.data.frame() %>%
  radarchart(
...
colpal <- c("tomato3", "darkslategrey") 
...
pdf(file.path("figures", "dbd_network_mode2_features.pdf"), width = 6, height = 6)
sumNet2 %>%
  t() %>% 
  as.data.frame() %>%
  radarchart(
...
# Mode 3 FC Visualization  -----------------------------------------------------
fconn_feature <- matrix(0, 268, 2)
fconn_feature[, 1] <- u.boot.plot[[dim.match[3]]]$load_nm[1:268 + 268 * 0] %>% abs()
fconn_feature[, 2] <- u.boot.plot[[dim.match[3]]]$load_nm[1:268 + 268 * 1] %>% abs()
shen_labels <- read_excel("Shen_idx.xls")
...
pdf(file.path("figures", "dbd_network_mode3_features.pdf"), width = 6, height = 6)
sumNet2 %>%
  t() %>% 
  as.data.frame() %>%
  radarchart(
...
maxEV_te <- ceiling(max(EVs_te) * 100) / 100 + 0.01
...
EVs_screeplot_te <- EVs_df_te %>%
  ...
  theme(legend.position = 'none')
EVs_screeplot_te
ggsave(file.path("figures", "dbd_scca_test_screeplot.png"), EVs_screeplot_te, width = 6, height = 4, dpi = 400)
...
label1 <- sprintf("italic(p[bonf]) < 0.001")
label2 <- sprintf("italic(p[bonf]) == %0.3f", scca_perm_te_pval_adj[scca_perm_te_pass[dim.match_te[2]]])
label3 <- sprintf("italic(p[bonf]) == %0.3f", scca_perm_te_pval_adj[1])
...
corrplot_te
ggsave(file.path("figures", "dbd_replication_corrplot.pdf"), corrplot_te, width = 4, height = 3)
...
date <- gsub("-", "", Sys.Date())
save.image(file = sprintf("%s_dbd_analysis.RData", date))
